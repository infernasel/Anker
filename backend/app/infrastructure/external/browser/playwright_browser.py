...
class PlaywrightBrowser:
    ...
    async def _extract_interactive_elements(self) -> List[str]:
        ...
        interactive_elements = await self.page.evaluate("""() => {
            ...
            for (let i = 0; i < elements.length; i++) {
                ...
                // Only add data-anker-id attribute to elements that meet the conditions
                element.setAttribute('data-anker-id', `anker-element-${validElementIndex}`);
                                                        
                // Build selector - using only data-anker-id
                const selector = `[data-anker-id="anker-element-${validElementIndex}"]`;
    ...
    async def _get_element_by_index(self, index: int) -> Optional[Any]:
        """Get element by index using data-anker-id selector
        ...
        # Use data-anker-id selector
        selector = f'[data-anker-id="anker-element-{index}"]'
